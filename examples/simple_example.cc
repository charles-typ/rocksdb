...
void generate_random_chars(char *buffer, size_t size) {
  const char charset[] =
      "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";
  size_t n = sizeof(charset) - 1;  // -1 to exclude the null-terminator

  for (size_t i = 0; i < size; i++) {
    buffer[i] = charset[rand() % n];
  }
...
DB *init_rocksdb(const string DBPath) {
  ...
  options.memtable_factory.reset(
      rocksdb::NewHashLinkListRepFactory(NUM_BUCKET));
  ...
  db->GetIntProperty("rocksdb.cur-size-active-mem-table", &active_mem);
  db->GetIntProperty("rocksdb.cur-size-all-mem-tables", &unflushed_mem);
  db->GetIntProperty("rocksdb.size-all-mem-tables", &all_mem);
  std::cout << "Memtable size is: " << active_mem << " " << unflushed_mem << " "
            << all_mem << std::endl;
  printf(
      "db_path: %s\nlogging level: %d\nmemtable size: %ldMB, num memtable: %d\n",
      DBPath.c_str(), options.info_log_level, MEMTABLE_SIZE >> 20,
      NUM_MEMTABLE);
...
static int is_write_rec_for_thread(uint64_t op_idx, uint64_t key, int node_id,
                                   int num_nodes, int thread_id,
                                   int num_threads) {
...
static uint64_t parse_load_ycsb(FILE *fp, uint64_t max_ops, int blade_id,
  ...
  while (op_idx < (max_ops / LOWER_DATASET_FACTOR)) {
    ...
    if (read == -1) break;  // end of the file
    ...
    if (line[0] == 'I') {
      ...
      if (strncmp(line, "INSERT", 6) == 0) {
        sscanf(line, "%s  user%lu", ops, &key_val);
        for (int thread_id = 0; thread_id < num_threads; thread_id++) {
          if (is_write_rec_for_thread(op_idx, key_val, blade_id, num_nodes,
                                      thread_id, num_threads)) {
            generate_random_chars(dummy_val, MAX_VALUE_SIZE - 1);
...
static void *run_rocksdb_ycsb(void *args) {
  ...
  if (thread_id == 0) {
    ...
    parse_load_ycsb(fp_ycsb_load, MAX_LOAD_OP_NUM, blade_id, num_node,
                    num_thread_per_blade);
    fclose(fp_ycsb_load);
    ...
    oplist = (struct hash_test_ycsb_ops *)mmap(NULL, file_size, PROT_READ,
                                               MAP_PRIVATE, fd_ycsb_run, 0);
    ...
    num_op = t_args->num_op =
        file_size / sizeof(struct hash_test_ycsb_ops) / num_thread_tot;
    if (num_op > MAX_RUN_OP_NUM / num_thread_tot)
      num_op = MAX_RUN_OP_NUM / num_thread_tot;
  ...
  if (cpid != 0) {
    ...
    */
    ...
    printf(
        "** Finished [thread: %d] %lu | %.3lf MOPS, finished in %.2lf ms, "
        "average latency %.2lf us\n",
        global_thread_id, num_op, ((double)num_op / t_tot / 1000), t_tot,
        (double)t_tot / (double)num_op * 1000);
...
static int launch_workers(int num_node, int num_thread, char *load_file,
  ...
  for (int tid = 0; tid < num_thread; ++tid) {
    for (int nid = 0; nid < num_node; ++nid) {
      ...
      std::cout << "thread id: " << tid << " node id: " << nid << std::endl;
  //    // res = pthread_create(&threads[i], NULL, run_rocksdb_ycsb,
  //    // &thread_args[i]);
      run_rocksdb_ycsb(&thread_args[i]);
      if (res) {
  ...
  if (total_time > 0) {
    printf("* Finished: num_op %lu | %.3lf MOPS\n", total_num_ops,
           (double)total_num_ops / total_time / 1000);
  } else {
...
int main(int argc, char *argv[]) {
  ...
  printf("num blades: %d, threads per blade: %d, tot threads: %d\n", num_nodes,
         num_threads_per_node, num_threads_tot);
  ...
  launch_workers(num_nodes, num_threads_per_node, argv[arg_load_file],
                 argv[arg_run_file]);
